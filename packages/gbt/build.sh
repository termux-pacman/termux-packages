...
termux_step_make() {
	...
	go mod init gbt
	go mod tidy
	go build -ldflags="-s -w -X main.version=$TERMUX_PKG_VERSION -X main.build=${_COMMIT::6}" \
		-o $TERMUX_PKG_MASSAGEDIR/$TERMUX_PREFIX/bin/gbt github.com/jtyr/gbt/cmd/gbt
}
termux_step_make_install() {
	mkdir -p $TERMUX_PKG_MASSAGEDIR/$TERMUX_PREFIX/{doc/gbt,share/gbt}
	cp -r $TERMUX_PKG_SRCDIR/{sources,themes} \
		$TERMUX_PKG_MASSAGEDIR/$TERMUX_PREFIX/share/gbt/
	cp -r $TERMUX_PKG_SRCDIR/{LICENSE,README.md} \
		$TERMUX_PKG_MASSAGEDIR/$TERMUX_PREFIX/doc/gbt/
}
