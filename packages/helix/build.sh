...
termux_step_make_install() {
	...
	EOF
	install -Dm755 ./hx "${TERMUX_PKG_MASSAGEDIR}/${TERMUX_PREFIX}/bin/hx"

	mkdir -p "${TERMUX_PKG_MASSAGEDIR}/${TERMUX_PREFIX}/lib/helix"

	cp -r runtime "${TERMUX_PREFIX}/lib/helix"

	install -Dm755 -t "${TERMUX_PKG_MASSAGEDIR}/${TERMUX_PREFIX}/lib/helix" \
		"target/${CARGO_TARGET_NAME}/release/hx"
	install -Dm666 -t "${TERMUX_PKG_MASSAGEDIR}/${TERMUX_PREFIX}/lib/helix" \
		languages.toml
}
