#!/bin/bash
#
#   repo-add.sh - add a package to a given repo database file
#   repo-remove.sh - remove a package entry from a given repo database file
#
...
# Import libmakepkg
source scripts/db/compress.sh
source scripts/db/message.sh
...
See %s(8) for more details and descriptions of the available options.\n")" $cmd
	printf "\n"
	if [[ $cmd == "repo-add.sh" ]] ; then
		printf -- "$(gettext "Example:  repo-add.sh /path/to/repo.db.tar.gz pacman-3.0.0-1-i686.pkg.tar.gz\n")"
	elif [[ $cmd == "repo-remove.sh" ]] ; then
		printf -- "$(gettext "Example:  repo-remove.sh /path/to/repo.db.tar.gz kernel26\n")"
	fi
}
...
if [[ $cmd != "repo-add.sh" && $cmd != "repo-remove.sh" ]]; then
